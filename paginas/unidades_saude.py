...
opcoes_tipos_unidades = [
    {"label": t, "value": t}
    for t in sorted(unidades_saude_df["CATEGORIA_UNIDADE"].dropna().unique())
]

opcoes_municipios_unidades = [
    {"label": m, "value": m}
    for m in sorted(unidades_saude_df["NO_MUNICIPIO"].dropna().unique())
]
...
def montar_mapa_unidades_saude(df):
    ...
    fig.update_layout(
        title="UBS, postos de saúde e UPAs municipais",
...
def detalhe_unidade(cnes, df):
    if not cnes or df.empty:
        return html.Div(
            "Clique em uma unidade no mapa para ver os detalhes.",
            style={"color": "#64748b"},
        )
    ...
    if row.empty:
        return html.Div(
            "Unidade não encontrada no filtro atual.",
            style={"color": "#64748b"},
        )
    ...
    return html.Div(
        [
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            dcc.Dropdown(
                                id="filtro-municipio-unidade",
                                options=opcoes_municipios_unidades,
                                multi=True,
                                placeholder="Todos",
                            ),
                        ],
            ...
            html.Div(
                [
                    html.Div(
                        dcc.Graph(id="mapa-unidades-saude"),
                        style={"flex": "2", "minWidth": "460px"},
                    ),
                    html.Div(
                        [
            ...
            html.Div(
                dcc.Graph(id="grafico-unidades-categoria"),
                style={**PANEL, "padding": "8px", "marginBottom": "14px"},
            ),

            html.Div(
                [
                    html.H3("Microdados das unidades", style={"marginTop": "0"}),
                    tabela("tabela-unidades-saude", page_size=15),
                ],
                style=PANEL,
            ),
        ]
...
def register_callbacks(app):
    ...
    def atualizar_unidades_saude(tipos, municipios, cnes_clicado):
        ...
        cards = [
            ...
            card("Clínicas da família", f"{int(por_tipo.get('Clinica da familia', 0))}", "categoria destacada no CNES/nome", "#0891b2"),
            card("UPA / PA", f"{int(por_tipo.get('UPA / Pronto atendimento', 0))}", "pronto atendimento e UPAs", "#dc2626"),
            card("Postos", f"{int(por_tipo.get('Posto de saude', 0))}", "postos de saúde identificados", "#f59e0b"),
        ]
        ...
        resumo = (
            dff.groupby("CATEGORIA_UNIDADE", as_index=False)
            .agg(UNIDADES=("CO_CNES", "count"))
        ...
        ).sort_values(["Categoria", "Município", "Unidade"])
